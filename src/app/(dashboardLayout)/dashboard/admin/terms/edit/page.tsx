...
import { getTermsConditions, saveTermsConditions } from "@/utils/termsConditions";
import { appAlert } from "@/utils/appAlert";

export default function AdminTermsConditionsEditor() {
  ...
  useEffect(() => {
    if (mounted && editorRef.current) {
      editorRef.current.innerHTML = getTermsConditions();
    }
  }, [mounted]);
  ...
  const handleSave = () => {
    ...
    try {
      ...
      saveTermsConditions(htmlContent);
      ...
      router.push("/dashboard/admin/terms");
    } catch (error) {
      appAlert.fire({
        icon: "error",
        title: "Save Failed",
        text: "Could not save Terms & Conditions. Please try again.",
      });
    } finally {
      setSaving(false);
    }
    ...
    <div className="max-w-4xl mx-auto pb-12 select-none">
      <div className="bg-white rounded-[32px] border border-gray-100 shadow-[0_8px_30px_rgba(0,0,0,0.015)] overflow-hidden">
        ...
        <div
          ...
          data-placeholder="Start writing terms and conditions..."
        />
